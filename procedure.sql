...
DELETE FROM wp_comments WHERE comment_post_ID=1

SELECT comment_ID, comment_content  FROM wp_comments wc 

call dorepeat_v2 (1,'новый комментарий');
...
BEGIN
  SET @x = 0;
  SET @post_id = (SELECT comment_post_ID from wp_comments group by comment_content, comment_post_ID having count(comment_content) >= 0 limit 1 );
  REPEAT SET @x = @x + 1; 
  INSERT INTO wp_comments (comment_post_ID,comment_author,comment_author_email,comment_author_url,comment_author_IP,comment_date,comment_date_gmt,comment_content,comment_karma,comment_approved,comment_agent,comment_type,comment_parent,user_id) VALUES
